...
function OsisFormatter(): OsisFormatterInterface {
  ...
  const osisFormat: RegExp =
    /^[1-5A-Za-z]{2,}(?:\.\d{1,3}(?:\.\d{1,3})?)?(?:-[1-5A-Za-z]{2,}(?:\.\d{1,3}(?:\.\d{1,3})?)?)?$/;
  ...
  function format(osisString: string, osisContext?: string | null): string {
  ...
  function formatToken(token: TokenType): string {
    ...
    for (let i: number = 0; i <= 1; i++) {
      ...
      const book: undefined | Array<string> = books[token[property]];
  ...
  function formatPart(token: TokenType, part: PartType): string {
    ...
    switch (part.type) {
      ...
      case "v":
        ...
        if (
          part.subType !== "" &&
          typeof options[part.subType] !== "undefined"
        ) {
      ...
      default:
        return formatVariable(
          getBestOption(part.type, part.subType, token.position),
          part,
          token,
        );
    }
  ...
  function formatVariable(
    ...
    pattern = pattern.replace(/\$chapters/g, function (): string {
      ...
      const arrayToUse: Array<string> =
        typeof books[`${part.b}.$chapters`] === "undefined"
          ? options.$chapters
          : books[`${part.b}.$chapters`];
    ...
    pattern = pattern.replace(/\$verses/g, function (): string {
      ...
      if (
        options.$verses.length > 1 &&
        hasMultipleVerses(part, token) === true
      ) {
  ...
  function annotateSequenceToken(
    token: TokenType,
    prevToken: TokenType,
    tokens: Array<TokenType>,
  ): void {
  ...
  function addBookSequence(
    ...
    while (sequenceArray.length > 1) {
      ...
      if (
        typeof books[bookSequence] !== "undefined" &&
        typeof books[bookSequence][0] === "string"
      ) {
  ...
  function osisWithContext(osis: string, context: ContextType): TokenType {
    ...
    const omitChapter: boolean =
      isSingleChapter &&
      typeof v === "string" &&
      (options.singleChapterFormat === "bv" ||
        options.singleChapterFormat === "b");
    ...
    if (
      osisChapterWithContext(b, c, v, omitChapter, context, token) === false
    ) {
      return token;
    }
  ...
  function osisBookWithContext(
    ...
    const returnSingleBook: boolean =
      v === undefined &&
  ...
  function osisChapterWithContext(
    b: string,
    c: string,
    v: string | null | undefined,
    omitChapter: boolean,
  ...
  function setContext(osis?: string | null): ContextType {
  ...
  function setOptions(userOptions: OptionsType): void {
    ...
    for (let i: number = 0, max: number = userKeys.length; i < max; i++) {
      ...
      if (
        ...
        typeof userOptions[key] === defaultType
      ) {
  ...
  function setBooks(userBooks: BooksType): void {
    ...
    books = {};
    Object.keys(userBooks).forEach(function (key: string): undefined {
      const value: Array<string> = userBooks[key];
      // Make sure it's an array...
      if (Array.isArray(value) === false) {
        throw `books["${key}"] should be an array: ${Object.prototype.toString.call(
          value,
        )}.`;
      }
